<script lang="ts">
	...
	function resolveGlobalError(errorKey: string): string {
		if (errorKey === 'createDeal.genericError') {
			return $translationStore.createDeal.genericError;
		}
...
<section class="mx-auto flex w-full max-w-5xl flex-col gap-6 py-6 sm:py-10">
	<div class="max-w-3xl space-y-3 px-1">
		...
		<h1 class="text-3xl font-semibold text-slate-950 sm:text-4xl">
			{$translationStore.createDeal.heading}
		</h1>
		<p class="max-w-2xl text-base leading-7 text-slate-600">
			{$translationStore.createDeal.intro}
		</p>
	</div>

	<Card class="overflow-hidden rounded-3xl border border-slate-200 bg-white p-0 shadow-sm">
			...
			<div class="space-y-5 rounded-3xl bg-slate-50 p-4 sm:p-5">
				...
				<div class="space-y-4 pt-2">
					...
					<Button type="submit" size="lg" class="w-full" disabled={$submitting}>
						{$submitting
							? $translationStore.createDeal.submitting
							: $translationStore.createDeal.submit}
					</Button>
				</div>
			</div>
		</form>
