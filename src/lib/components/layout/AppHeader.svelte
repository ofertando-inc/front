...
<Navbar class="border-b border-orange-100 bg-white/90 px-4 py-3 backdrop-blur">
	...
	{#snippet default({ hidden, toggle })}
		<div class="flex items-center gap-2 lg:order-2">
			<div class="hidden min-w-32 sm:block">
				<Select on:change={handleLocaleChange} value={$localeStore}>
			...
			{#if $authStore.isAuthenticated && $authStore.user}
				...
				<Dropdown triggeredBy="#user-menu">
					...
					<DropdownDivider />
					<DropdownItem on:click={handleLogout}>{$translationStore.common.logout}</DropdownItem>
				</Dropdown>
			{:else}
