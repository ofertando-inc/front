...
- Merchant + Location domain for the geo evolution, replacing the flat `store` concept: a brand-level **merchant** (verifiable, shared by online and physical offers) and a **location** (a verifiable address owned by a merchant, many per merchant). New `merchant` / `geocoding` API clients (`searchMerchants`, `getMerchant`, `createMerchant`, `geocode` over `GET /geocode`) with `MerchantResponse` / `LocationResponse` / `OfferMerchant` / `OfferLocation` / `GeocodeSuggestion` types, and `merchant.not_found` / `merchant.merge_invalid` / `location.not_found` / `offer.location_required` / `offer.invalid_near` error keys localized in the three languages. Unit-tested.
- Offer form reworked around merchants and addresses: a `MerchantCombobox` (autocomplete over `GET /merchants` — verified or already-used merchants — free text find-or-creates on submit) and, for physical offers, a **city** field (autocomplete constrained to the bundled Colombian-cities list, so the value stays clean — "Cali", not the geocoder's "Cali ciudad") plus an `AddressCombobox` that geocodes the address (`GET /geocode`, biased by the chosen city, with clean street + city/region suggestions); picking a suggestion keeps the exact address typed and only anchors the region/coordinates — the city is never derived from the geocoder. The flat form fields are reshaped server-side into the nested `CreateOfferDto` (`merchantId` | `merchantName`, `locationId` | `location{}`); online offers omit the location and require an external link. An "online" checkbox toggles the channel, and the physical location cascades — entering the city reveals the address and the validity dates. Array-level field errors (e.g. a missing category) now surface under their field. New `createDeal.merchant*` / `city*` / `address*` keys and `deals.verified` / `nearMe` keys in the three languages; zod schema, server mapping, and e2e coverage.
- Interactive map (Leaflet + OpenStreetMap, client-only) for the physical location, shown once an address is entered: a `LocationMapPicker` on the offer form lets the user drop and drag a pin to set the exact coordinates (the pin recenters when a geocode suggestion is picked; dragging it reverse-geocodes — `GET /geocode/reverse` — to refresh the region, leaving the chosen city and the free-text address untouched), and a read-only `LocationMap` shows the pinned location on the deal detail page. New `reverseGeocode` API client (unit-tested), `createDeal.mapHint` and `deal.locationTitle` keys in the three languages, and an end-to-end "publish a local offer" e2e smoke.

### Changed

- Split the offer's channel and nature into two independent fields, matching the backend model: a new `isOnline` boolean (online vs physical) and the existing free-text `offerType`, now repurposed as the promotion _nature_ (discount / 2x1 / coupon / cashback / clearance / free shipping / other) surfaced through a curated `offerNature` select. There is no offer-level `city` anymore — `location.city` is the single source of truth for a physical offer's city (the front never reads a denormalized offer city), `Offer.isOnline` is added, and the deal card / detail now show the merchant (with a verified badge) and the location, keying the channel badge off `isOnline`. The listing filters drop the store filter for a channel filter (`?online`) plus a "near me" geolocation button (`?near` / `radiusKm`), and the facets lose their `stores` bucket. New `offer.online_requires_url` error key and `createDeal.offerTypePlaceholder` / `offerNature` / `isOnlineLabel` / `isOnlineHint` / `externalUrlHintOnline` keys localized in the three languages (the old `offerTypeOnline` / `offerTypeLocal` strings, the `Nacional` city sentinel, and the `store`/`offerType` query filters were removed); zod schema, server form mapping, and e2e updated.
